...
class IKController:
    def __init__(self, T_wb, q, obstacles):
        ...
        self.ngens = (30, 50, 100)
        self.pops = (16, 32, 64)
        self.ftols = (5e-3, 1e-4, 5e-5, 1e-5, 5e-6)
        for sig in self.sigs:
            for ngen in self.ngens:
                for pop in self.pops:
                    for ftol in self.ftols:
                        self.hyper_dict[('sig: ' + str(sig), 'ngen: ' + str(ngen), 'pop: ' + str(pop), 'ftol: ' + str(ftol))] = \
                        {'time': 0., 'error': 0.}
    ...
    def solve(self, q, obstacles, target_pos):
        ...
        #                     "sigma": sig,                   # standard deviation of initial population
        #                     "ngen": ngen,                   # max number of generations
        #                     "popsize": pop,                 # population size (lambda)
        #                     "ftol": ftol,                   # tolerance of f value (error). Algorithm stops early once the mean improvement per generation is below this value.
        #                     "bounds": self.jnt_bounds       # Joint limit constraints
        #                 }
        #
        #                 st = time.perf_counter()
        ...
        #                 q_res, f_res = solve_ik(q, obstacles, target_pos, options)
        #
        #                 self.hyper_dict[('sig: ' + str(sig), 'ngen: ' + str(ngen), 'pop: ' + str(pop), 'ftol: ' + str(ftol))]['time'] += time.perf_counter() - st
        #                 self.hyper_dict[('sig: ' + str(sig), 'ngen: ' + str(ngen), 'pop: ' + str(pop), 'ftol: ' + str(ftol))]['error'] += f_res
        ...
        # pop = 256
        #
        # cma = CMA(centroid=self.current_q, sigma=sig, lambda_=pop)
        #
        # cached_pop = []
        # st = time.perf_counter()
        ...
        # print('Value: ', obj_fun(q_res_feasible))
